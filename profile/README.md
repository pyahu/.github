...
| # | Step | Tool | Goal |
| --- | --- | --- | --- |
| 1 | Prepare | [Toolchain](#pyahu-toolchain) | Get the team on the same page |
| 2 | Create | [Initializer](#pyahu-initializer-coming-soon) | Start with the right foundations |
| 3 | Develop | [AI Factory](#pyahu-ai-factory-coming-soon) | Build with AI and keep the discipline |
| 4 | Validate | [CLI](#pyahu-cli) | Make local development real |
| 5 | Operate | [Open Cluster Foundation](#pyahu-open-cluster-foundation) and [Cloud](#pyahu-cloud-enterprise) | Build infrastructure you understand |

## Community projects

| Project | What it does | Status |
| --- | --- | --- |
| [**Toolchain**](https://github.com/pyahu/toolchain) | Curated developer tools managed by mise, with consistent versions on every workstation | ✅ Available |
| [**CLI**](https://github.com/pyahu/cli) | Local Kubernetes (k3d) with the services your app needs, in one command | ✅ Available |
| [**Open Cluster Foundation**](https://github.com/pyahu/open-cluster-foundation) | Production-grade Kubernetes from zero: Terraform plus an observable service layer | ✅ OCI available |
...
and managed with [mise](https://mise.jdx.dev). No dev container and no per-language version
managers. You get a small cross-stack baseline plus the workflow profiles you choose:
`workstation`, `java`, `go`, `python`, `node`, `cloud`, `ai` and `arch`.
...
Repository: [github.com/pyahu/toolchain](https://github.com/pyahu/toolchain)
...
PostgreSQL, ZITADEL (OIDC over local HTTPS), RabbitMQ, Valkey, Kafka, Kafka Connect with Debezium
and Kafka UI, all declared in a single `pyahu.yaml`.
...
Docs: [cli.pyahu.io](https://cli.pyahu.io). Repository: [github.com/pyahu/cli](https://github.com/pyahu/cli)
...
is pinned and validated in CI. OCI/OKE today, with more providers on the roadmap.

Repository: [github.com/pyahu/open-cluster-foundation](https://github.com/pyahu/open-cluster-foundation)
...
> [!IMPORTANT]
> **Not production-ready yet.** AI Factory is in pre-release: the local workflow runs, but commands
> and contracts may still change, and capabilities such as dispatch mode are still planned. Launch is
> coming soon at **ai.pyahu.io**.

**AI moves fast. Give it an engineering process.** A harness powered by Pi as the model runtime,
with an Extreme Programming workflow, curated skills and specialized agents. Turn a story into an
explicit agreement, work through the test cycle and review the result, while your team sets the
...
**Pyahu Cloud** gives your code a place to run, with managed delivery and operations, isolated
environments, PostgreSQL and observability. It is JVM-first and comes with guided adoption, an
adoption path built around your team and a direct relationship with the people building Pyahu.
